...
: mName("UNNAMED"), mPosition(0.0f, 0.0f, 0.0f), mScaling(1.0f, 1.0f, 1.0f), mRotationAxis(0.0f, 1.0f, 0.0f),
  mRotationAngleInDegrees(0.0f), mAnimation(nullptr),mMaterial(1.0f, 1.0f, 1.0f,1.0f),mAttenuation(1.0f, 1.0f, 1.0f)
{
}
...
bool Model::ParseLine(const std::vector<ci_string> &token)
{
    if (token.empty() == false)
	{
        ...
        else if (token[0] == "particleemitter")
        {
			...
			mMaterial.y = static_cast<float>(atof(token[3].c_str()));
			mMaterial.z = static_cast<float>(atof(token[4].c_str()));
			mMaterial.w = static_cast<float>(atof(token[5].c_str()));
		}
